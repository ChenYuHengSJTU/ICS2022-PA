...
void _exit(int status) {
  _syscall_((intptr_t)SYS_exit, status, 0, 0);
  while (1);
}

int _open(const char *path, int flags, mode_t mode) {
  // _exit(SYS_open);
  int fd=_syscall_((intptr_t)SYS_open,(intptr_t)path,flags,mode);
  return fd;
}

int _write(int fd, void *buf, size_t count) {
  // _exit(SYS_write);
  return _syscall_((intptr_t)SYS_write,fd,(intptr_t)(buf),count);
}
...
int _read(int fd, void *buf, size_t count) {
  // _exit(SYS_read);
  return _syscall_((intptr_t)SYS_read,fd,(intptr_t)buf,count);
}

int _close(int fd) {
  // _exit(SYS_close);
  return _syscall_((intptr_t)SYS_close,fd,0,0);
}

off_t _lseek(int fd, off_t offset, int whence) {
  // _exit(SYS_lseek);
  return _syscall_((intptr_t)SYS_lseek,fd,offset,whence);
}
